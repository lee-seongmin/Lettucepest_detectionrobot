...
int value = 45;
int ANGLE = 45; // 다른 방향에서 직진으로 바뀔 때 돌아와야할 각도
...
void motor_on(int ANGLE, int JET)
{
  JET = digitalRead(JET_PUMP);
  
  if(isMotorOn== false && JET == HIGH) 
  {
    // 모터가 꺼져있는 경우
    motor_speed(0);
    delay(250);
    digitalWrite(EN_PUMP,HIGH);
    digitalWrite(IN3_PUMP, HIGH);    // 모터 방향 설정
    digitalWrite(IN4_PUMP, LOW);
    analogWrite(EN_PUMP, 255);       // 모터 속도 설정 (0~255)
    delay(750);
    isMotorOn = true;            // 모터 상태 업데이트
